...
for (let cont = 0; cont < siteNames.length; cont++) {
    ...
    for (let cd = 0; cd < costData.length; cd++) {
        var mid = []
        
        for (let cdc = 0; cdc < costData[cd].length; cdc++) {
            // console.log(costData[cd].length)
            var newValue = costData[cd][cdc]*2.3
            // console.log(newValue)
            mid.push(newValue)
        }

        CostDataMult.push(mid)
    ...
    for (let c = 0; c < arrowList.length; c++) {
        ...
        scaleFactorY = getRandomArbitrary(0.025,0.05)
        
        if (arrowList[c] != null){
            arrowList[c].setAttribute('transform', 'rotate(90) scale(' + CostDataMult[cont][c] + ')')  
            // console.log(CostDataMult[cont][c])
            // arrowlist[c].setAttribute('transform', 'rotate(90) scale(' + 0.02 + ')')   
        }
        // }

        if (windList[c] != null){ 
            windList[c].setAttribute('transform', 'rotate(90) scale(' + CostDataMult[cont][c] + (scaleFactorY*1.3) + ')')    
            // windList[c].setAttribute('transform', 'rotate(90) scale(' + CostDataMult[cont][c] + ')') 
